    ...
    while (chances > 0):
      ...
      for chute in chutes:
        if chute in palavra_sorteada:
          indices = [
            i for i in range(len(palavra_sorteada))
            if palavra_sorteada.startswith(chute, i)
          ]
          for index in indices:
            posicoes[index] = palavra_sorteada[index]
          print(
            f'Certo!, a letra "{chute.capitalize()}" aparece nas posições:\n {posicoes}'
          )
        else:
      ...
      # Se as chances acabarem
        ...
        print(
          f'Mongão, chances acabaram\n A palavra era: {palavra_sorteada.title()}'
        )

  verificar_chute(
    sorteiar_palavra(gerar_palavras_secretas_e_categoria(palavras, coluna)),
    chances, palavras, coluna)
